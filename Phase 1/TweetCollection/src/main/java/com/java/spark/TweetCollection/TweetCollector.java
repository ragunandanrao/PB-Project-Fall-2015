...
StringBuilder sb;
    public void run(){
        ...
        tweetRequest.addHeader("user-agent", "Twitter Stream Reader");
         tweetRequest.addBodyParameter("track","world"); // Keyword to be tracked
        tweetRequest.addBodyParameter("language", "en");
        ...
        if(tweet!=null)
        {
        	tweet=((((tweet.replaceAll("@","")).replaceAll("#", ""))).replaceAll("\""," ")).replaceAll(":", " ");
       ...
       { 
    	   try{
    	   File tweetFile = new File("/home/nishit/Project/TweetFile.txt");
           if(!tweetFile.exists())
           {
